...
| `phone_verification_screen_test.dart` | первый кадр — крутилка, а не форма (нет мигания); локально привязан → сразу «уже подключили»; локально нет, сервер `bound:true` → подключённое состояние; сервер `bound:false` → форма ввода; короткий заголовок AppBar |
